const { splitRangeIntoTwoPartsIfItIsCircular, getSequenceWithinRange, invertRange, isPositionWithinRange} = require('ve-range-utils');
const spliceString = require('string-splice');

module.exports = function adjustBpsToReplaceOrInsert(bpString, insertString = "", caretPositionOrRange) {
    let stringToReturn = bpString
    
    if (caretPositionOrRange && caretPositionOrRange.start > -1) {
        // invertRange()
        // getSequenceWithinRange()
        const ranges = splitRangeIntoTwoPartsIfItIsCircular(invertRange(caretPositionOrRange, bpString.length))
        stringToReturn = ""
