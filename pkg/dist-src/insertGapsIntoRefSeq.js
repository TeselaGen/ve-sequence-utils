import getAllInsertionsInSeqReads from './getAllInsertionsInSeqReads.js'; // seqReads should be an array of objects [{name, seq, pos, cigar}, {name, seq, pos, cigar}, ...]
...
export default function insertGapsIntoRefSeq(refSeq, seqReads) {
  ...
  const allInsertionsInSeqReads = getAllInsertionsInSeqReads(seqReads); // console.log("all insertions", allInsertionsInSeqReads)

  for (let i = 0; i < allInsertionsInSeqReads.length; i++) {
    const bpPosOfInsertion = allInsertionsInSeqReads[i].bpPos;
    const numberOfInsertions = allInsertionsInSeqReads[i].number; // adding gaps at the bp pos of insertion
