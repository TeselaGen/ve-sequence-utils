...
import { ambiguous_dna_values, extended_protein_values } from "./bioData.js";
import getReverseComplementSequenceString from "./getReverseComplementSequenceString.js";
export default function findSequenceMatches(sequence, searchString, options = {}) {
  ...
  if (searchReverseStrand) {
    ...
    const reverseMatches = findSequenceMatchesTopStrand(reverseSeq, searchString, options);
    const flippedReverseMatches = reverseMatches.map(range => {
      return { ...flipContainedRange(range, {
          start: 0,
...
function convertAmbiguousStringToRegex(string, isProtein) {
  ...
  return reduce(string, (acc, char) => {
    const value = isProtein ? extended_protein_values[char.toUpperCase()] : ambiguous_dna_values[char.toUpperCase()];
    if (!value) return acc;
