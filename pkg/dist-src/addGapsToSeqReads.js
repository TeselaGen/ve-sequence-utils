...
import { cloneDeep } from 'lodash'; // bam.seq: NTGTAAGTCGTGAAAAAANCNNNCATATTNCGGAGGTAAAAATGAAAA...
// bam.pos: 43
...
export default function addGapsToSeqReads(refSeq, seqReads) {
  ...
  const refSeqWithGaps = insertGapsIntoRefSeq(refSeq.sequence, seqReads); // first object is reference sequence with gaps, to be followed by seq reads with gaps

  let seqReadsWithGaps = [{
    name: refSeq.name,
    sequence: refSeqWithGaps.toUpperCase()
  }];
  seqReads.forEach(seqRead => {
    ...
    seqReads.forEach(seqRead => {
      ...
      const splitSeqRead = seqRead.cigar.match(/([0-9]*[SMDI])/g); // adjust seqRead.pos, aka bp pos where the seq read starts aligning to the ref seq, if bps have been soft-clipped from the beginning of the seq read
    ...
    eachSeqReadWithGaps = eachSeqReadWithGaps.join("").split(""); // 2) add own deletions to own sequence
    // get own deletions

    let ownDeletions = [];

    for (let componentI = 0; componentI < splitSeqReadChunk.length; componentI++) {
      if (splitSeqReadChunk[componentI].slice(-1) === "D") {
        let bpPosOfDeletion = adjustedSeqReadPos;
        const numberOfDeletions = Number(splitSeqReadChunk[componentI].slice(0, -1));

        for (let i = 0; i < componentI; i++) {
          const previousComponentNumber = Number(splitSeqReadChunk[i].slice(0, -1));
    ...
    for (let ownD = 0; ownD < sortedOwnDeletions.length; ownD++) {
      ...
      const numberOfDeletions = sortedOwnDeletions[ownD].number; // adding gaps at the bp pos
    ...
    eachSeqReadWithGaps = eachSeqReadWithGaps.join("").split(""); // 3) remove own insertions from own sequence
    // get own insertions
    ...
    for (let componentI = 0; componentI < splitSeqReadChunk.length; componentI++) {
      if (splitSeqReadChunk[componentI].slice(-1) === "I") {
        let bpPosOfInsertion = adjustedSeqReadPos;
        const numberOfInsertions = Number(splitSeqReadChunk[componentI].slice(0, -1));
        let nucleotides = [];
    ...
    let ownInsertionsCompare = JSON.parse(JSON.stringify(ownInsertions)); // sort own insertions by ascending bp pos
    ...
    otherInsertions = otherInsertions.filter((object, index) => index === otherInsertions.findIndex(obj => JSON.stringify(obj) === JSON.stringify(object))); // 'i < otherInsertions.length - 1' because when at the end of the array, there is no 'i + 1' to compare to
    ...
    for (let otherI = 0; otherI < ownInsertionsCompare.length; otherI++) {
      let insertionInfoIndex = otherInsertions.findIndex(e => e.bpPos === ownInsertionsCompare[otherI].bpPos);

      if (insertionInfoIndex !== -1) {
        if (otherInsertions[insertionInfoIndex].number > ownInsertionsCompare[otherI].number) {
          otherInsertions[insertionInfoIndex].number = otherInsertions[insertionInfoIndex].number - ownInsertionsCompare[otherI].number;
        } else if (otherInsertions[insertionInfoIndex].number <= ownInsertionsCompare[otherI].number) {
    ...
    for (let overlapI = 0; overlapI < sortedOwnInsertions.length; overlapI++) {
      let insertionInfoIndex = otherInsertions.findIndex(e => e.bpPos === sortedOwnInsertions[overlapI].bpPos);

      if (insertionInfoIndex !== -1) {
        if (otherInsertions[insertionInfoIndex].number > sortedOwnInsertions[overlapI].number) {
          otherInsertions[insertionInfoIndex].number = otherInsertions[insertionInfoIndex].number - sortedOwnInsertions[overlapI].number;
        } else if (otherInsertions[insertionInfoIndex].number <= sortedOwnInsertions[overlapI].number) {
          otherInsertions.splice(insertionInfoIndex, 1);
          overlapI--;
        }
      }
    ...
    let adjustedOwnInsertionsBp = JSON.parse(JSON.stringify(sortedOwnInsertionsBp));
    ...
    for (let ownI = 0; ownI < adjustedOwnInsertionsBp.length; ownI++) {
      ...
      adjustedOwnInsertionsBp[ownI].bpPos = adjustedOwnInsertionsBp[ownI].bpPos - previousInserts;
      sortedOwnInsertionsBp[ownI].bpPos = sortedOwnInsertionsBp[ownI].bpPos - previousInserts;
    }

    for (let otherI = 0; otherI < otherInsertions.length; otherI++) {
      for (let ownI = 0; ownI < adjustedOwnInsertionsBp.length; ownI++) {
        if (otherInsertions[otherI].bpPos <= sortedOwnInsertionsBp[ownI].bpPos) {
    ...
    for (let otherI = 0; otherI < otherInsertions.length && otherInsertions[otherI].bpPos <= eachSeqReadWithGaps.length; otherI++) {
      const bpPosOfInsertion = otherInsertions[otherI].bpPos;
      const numberOfInsertions = otherInsertions[otherI].number; // adding gaps at the bp pos
  ...
  seqReads.forEach(seq => {
    ...
    if (splitSeqReadChunk[0].slice(-1) === "S") {
      // # in #S at beginning of array, i.e. number of soft-clipped base pairs at beginning of the seq read
      const numOfBeginningSoftClipped = splitSeqReadChunk[0].slice(0, -1);
      adjustedSeqReadPos = seq.pos - numOfBeginningSoftClipped; // number of gaps to add if soft-clipped reads extend beyond beginning of ref seq
  ...
  for (let i = 1; i < seqReadsWithGaps.length; i++) {
    ...
    let adjustedSeqReadPos = cloneDeep(seqReads[i - 1].pos); // longest length of bps that extend beyond the beginning of the ref seq among all seq reads
  ...
  if (lengthsOfLongerSeqReads.length > 0) {
    ...
    for (let i = 0; i < seqReadsWithGaps.length; i++) {
      if (seqReadsWithGaps[i].sequence.length < longestSeqReadLength) {
        seqReadsWithGaps[i].sequence += "-".repeat(longestSeqReadLength - seqReadsWithGaps[i].sequence.length);
      }
    }
  ...
  for (let i = 1; i < seqReadsWithGaps.length; i++) {
    ...
    if (seqReadsWithGaps[i].sequence.length < refSeq.sequence.length) {
      seqReadsWithGaps[i].sequence += "-".repeat(refSeq.sequence.length - seqReadsWithGaps[i].sequence.length);
    }
