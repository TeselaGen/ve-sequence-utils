...
import revComp from "./getReverseComplementSequenceString.js";
import getAA from "./getAminoAcidFromSequenceTriplet.js"; // var ac = require('ve-api-check');

import proteinAlphabet from "./proteinAlphabet.js"; // ac.throw([ac.string,ac.bool],arguments);
...
export default function getAminoAcidDataForEachBaseOfDna(originalSequenceString, forward, optionalSubrangeRange, isProteinSequence) {
  ...
  let sequenceStringLength = isProteinSequence ? sequenceString.length * 3 : sequenceString.length; // ac.throw([ac.string,ac.bool],arguments);
  ...
  if (!forward) {
    ...
    aminoAcidIndex = Math.floor((sequenceStringLength - 1) / 3); //because we're translating in the reverse direction, we need to
