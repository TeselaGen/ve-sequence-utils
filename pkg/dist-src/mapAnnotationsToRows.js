...
import { each } from 'lodash'; // const areNonNegativeIntegers = require('validate.io-nonnegative-integer-array');
...
function mapAnnotationToRows({
  ...
  const ranges = splitRangeIntoTwoPartsIfItIsCircular(location || annotation, sequenceLength);
  ranges.forEach(function (range, index) {
    // if (!isPositiveInteger(range.start)) {}
    const startingRow = Math.floor(range.start / bpsPerRow);
    const endingRow = Math.floor(range.end / bpsPerRow); // const numberOfRows = endingRow - startingRow + 1;
    ...
    for (let rowNumber = startingRow; rowNumber <= endingRow; rowNumber++) {
      ...
      const start = rowNumber === startingRow ? range.start : rowNumber * bpsPerRow;
      const end = rowNumber === endingRow ? range.end : rowNumber * bpsPerRow + bpsPerRow - 1;
