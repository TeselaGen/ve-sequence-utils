...
export default function getDigestFragmentsForCutsites(sequenceLength, circular, cutsites) {
  const fragments = [];
  ...
  pairs.forEach(([cut1, cut2]) => {
    ...
    const size = getRangeLength(fragmentRange, sequenceLength); // const id = uniqid()

    const id = start + "-" + end + "-" + size + "-"; // getRangeLength({ start, end }, sequenceLength);
