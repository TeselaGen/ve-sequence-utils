...
import convertDnaCaretPositionOrRangeToAa from "./convertDnaCaretPositionOrRangeToAA.js";
...
import { modifiableTypes } from "./annotationTypes.js";
import adjustBpsToReplaceOrInsert from "./adjustBpsToReplaceOrInsert.js";
export default function insertSequenceDataAtPositionOrRange(_sequenceDataToInsert, _existingSequenceData, caretPositionOrRange) {
  ...
  let caretPosition = caretPositionOrRange; //update the sequence
  ...
  newSequenceData.proteinSequence = adjustBpsToReplaceOrInsert(existingSequenceData.proteinSequence, sequenceDataToInsert.proteinSequence, convertDnaCaretPositionOrRangeToAa(caretPositionOrRange)); // if (existingSequenceData.isProtein) {
  // }
  //handle the insert

  modifiableTypes.forEach(annotationType => {
    let existingAnnotations = existingSequenceData[annotationType]; //update the annotations:
    ...
    if (caretPositionOrRange && caretPositionOrRange.start > -1) {
      ...
      caretPosition = range.start; //update all annotations for the deletion
    ...
    newSequenceData[annotationType] = []; //in two steps adjust the annotations to the insert
  ...
  return map(annotationsToBeAdjusted, function (annotation) {
    return { ...adjustRangeToInsert(annotation, insertStart, insertLength),
      ...(annotation.locations && {
        locations: annotation.locations.map(loc => adjustRangeToInsert(loc, insertStart, insertLength))
      })
    };
  });
}
...
function adjustAnnotationsToDelete(annotationsToBeAdjusted, range, maxLength) {
  return map(annotationsToBeAdjusted, function (annotation) {
    ...
    const newLocations = annotation.locations && annotation.locations.map(loc => adjustRangeToDeletionOfAnotherRange(loc, range, maxLength)).filter(range => !!range);
