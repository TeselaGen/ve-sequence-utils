...
export default function getSequenceDataBetweenRange(seqData, range, options = {}) {
  ...
  let seqDataToReturn = extend({}, seqDataToUse, {
...
function getAnnotationsBetweenRange(annotationsToBeAdjusted, range, maxLength, shouldExcludePartial) {
  return flatMap(annotationsToBeAdjusted, function (annotation) {
    ...
    const overlaps = getZeroedRangeOverlaps(annotation, range, maxLength).map(overlap => {
