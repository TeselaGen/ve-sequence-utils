...
import getReverseComplementSequenceString from "./getReverseComplementSequenceString.js"; // const ac = require('ve-api-check');
...
export default function getOrfsFromSequence(options) {
  ...
  const re = useAdditionalOrfStartCodons ? /(?=((?:A[TU]G|G[TU]G|C[TU]G)(?:.{3})*?(?:[TU]AG|[TU]AA|[TU]GA)))/gi : /(?=((?:A[TU]G)(?:.{3})*?(?:[TU]AG|[TU]AA|[TU]GA)))/gi; // const str = 'tatgaatgaatgffffffatgfftaaftaafatgfatgfffffsdfatgffatgfffstaafftaafffffffffffffffatgtaaataa\n\natgffftaaf\n\natgffatgftaafftaa\n\natgatgftaafftaa\n\natgatgtaataa\n\ntttttttttttttaatgatgfffffffffftaa';

  let m;
  const orfRanges = []; //loop through orf hits!
  ...
  while ((m = re.exec(sequence)) !== null) {
    ...
    if (orfLength >= minimumOrfSize) {
      ...
      const start = m.index;
      let end = orfLength + start - 1; //normalize the end if it is greater than the original sequence length
  ...
  orfRanges.forEach(function (orf, index) {
    ...
    if (typeof indexOfAlreadyExistingOrf !== "undefined") {
      const internalStartCodonIndex = forward ? orf.start : originalSequenceLength - orf.start - 1; //use either the start or the end depending on the direction of the orf
      //we know because of how the regex works that larger orfs come first in the array

      orfRanges[indexOfAlreadyExistingOrf].internalStartCodonIndices.push(internalStartCodonIndex); //set a flag that we'll use to remove all these shorter, duplicated orfs
