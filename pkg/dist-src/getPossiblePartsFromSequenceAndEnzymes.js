...
export default function getPossiblePartsFromSequenceAndEnzyme(seqData, restrictionEnzymes) {
  // ac.throw([
  //     ac.string,
  //     ac.bool,
  //     ac.shape({
  ...
  if (cutsites.length < 1) {
    ...
    pairs.forEach(function (pair) {
      let cut1 = pair[0];
      let cut2 = pair[1];
      let part1 = getPartBetweenEnzymesWithInclusiveOverhangs(cut1, cut2, seqLen);
      let part2 = getPartBetweenEnzymesWithInclusiveOverhangs(cut2, cut1, seqLen);
...
function getPartBetweenEnzymesWithInclusiveOverhangs(cut1, cut2, seqLen) {
  ...
  let end = normalizePositionByRangeLength((cut2.topSnipBeforeBottom ? cut2.bottomSnipPosition : cut2.topSnipPosition) - 1, seqLen);
  return {
    start,
    start1Based: start + 1,
    end,
